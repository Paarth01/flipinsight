def test_recommend_search():
    ...
    r = client.post("/recommend/search", json={"query": "waterproof running shoes for men", "k": 5})
...
def test_recommend_similar_by_id():
    ...
    r = client.get(f"/recommend/similar/{uid}?k=5")
    assert r.status_code == 200
    data = r.json()
...
def test_recommend_similar_not_found():
    from tests.test_api import client
    r = client.get("/recommend/similar/nonexistent-id-999?k=5")
    assert r.status_code == 404


def test_recommend_search_k_bounds():
    from tests.test_api import client
    r = client.post("/recommend/search", json={"query": "test", "k": 100})
